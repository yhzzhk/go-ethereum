...
func (srv *Server) setupLocalNode() error {
	...
	db, err := enode.OpenDB(srv.Config.NodeDatabase) //打开本地数据库，具体路径为（/home/yhz/.ethereum/geth/nodes）
	//log.Info("-----------------------------nodedatabse path:", srv.Config.NodeDatabase)
...
func (srv *Server) setupDiscovery() error {
	...
	//在这里，混合器用于寻找对等体（peers），可能有多个不同的源提供候选节点。
	//在寻找对等体时，混合器会等待一个时间段，即 timeout 的值，来尝试从一个源中获取候选节点。
	//如果在 timeout 时间内没有获取到候选节点，混合器会尝试从其他源获取节点。
	...
	if !srv.NoDiscovery {
		...
		cfg := discover.Config{
			...
			NetRestrict: srv.NetRestrict,    //允许ip地址列表
			Bootnodes:   srv.BootstrapNodes, //引导节点列表
			Unhandled:   unhandled,
			Log:         srv.log,
		}
...
func (srv *Server) setupListening() error {
	...
	log.Info("---------------------------------------------srv.ListenAddr:", srv.ListenAddr)
	if err != nil {
		return err
	}
...
func (srv *Server) run() {
	...
	//定义变量 peers和trusted都是map类型的数据，即：peers[enode.ID]是*Peer类型
	//trusted[enode.ID]是bool类型，目前按照trustednodes的数量初始化trusted，并全部设置为true
	var (
