...
func (cn *cqlconnection) CreateOrUpdateNode(ctx context.Context, id string, ip string, udpport int, tcpport int, iffindnode bool, enr string) (string, error) {
    ...
    cql := `
    MERGE (n:Node {id: $id})
    ON CREATE SET n.ip = $ip, n.udpport = $udpport, n.tcpport = $tcpport, n.iffindnode = $iffindnode, n.enr = $enr
    ON MATCH SET n.ip = $ip, n.udpport = $udpport, n.tcpport = $tcpport, n.iffindnode = CASE WHEN $iffindnode THEN true ELSE n.iffindnode END, n.enr = $enr
...
func (cn *cqlconnection) ProcessInteractionNode(ctx context.Context, id string, ip string, udpport int, tcpport int, enr string) (string, error) {
	// 调用 CreateOrUpdateNode 以创建或更新节点，将 iffindnode 设置为 true
	return cn.CreateOrUpdateNode(ctx, id, ip, udpport, tcpport, true, enr)
}

func (cn *cqlconnection) ProcessNeighborNode(ctx context.Context, interactionNodeId string, neighborId, neighborIp string, neighborUdpPort, neighborTcpPort int, neighborEnr string, neighborDistance int) error {
	// 首先尝试创建或更新邻居节点，这里假设 iffindnode 对于邻居节点是 false
	_, err := cn.CreateOrUpdateNode(ctx, neighborId, neighborIp, neighborUdpPort, neighborTcpPort, false, neighborEnr)
	if err != nil {
		return err
	}
