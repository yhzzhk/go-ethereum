...
func (cn *cqlconnection) CreatNode(ctx context.Context, id string, ip string, iflive bool, isdiscv5 bool) (string, error) {
	...
	cql := "CREATE (a:Node {id:$id, ip:$ip, ping:$ping, isdiscv5:$isdiscv5}) RETURN a.id + ', from node ' + id(a) "

	node, err := session.ExecuteWrite(ctx, func(transaction neo4j.ManagedTransaction) (any, error) {
		result, err := transaction.Run(ctx,
			cql, map[string]any{"id": id, "ip": ip, "ping": iflive, "isdiscv5": isdiscv5})
		if err != nil {
			return "", err
		}
